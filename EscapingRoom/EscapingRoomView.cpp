...
void CEscapingRoomView::DrawGLScene(void) {
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	Vec3 pLoc = CEscapingRoomView::game.getPlayer()->location + Vec3(0.5, 1.0, 0.5);
	gluPerspective(45.0f, 2.0f, abs((pLoc - CEscapingRoomView::game.getCurrentWorld()->camera).dot(CEscapingRoomView::game.getCurrentWorld()->eye.front)) - 0.5f, 1000.0f);
	glMatrixMode(GL_MODELVIEW);
